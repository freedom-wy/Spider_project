...
class EnvironmentalPenalty(BaseSpider):
    """
    环保处罚爬虫
    """
    ...
    async def detail_one_parse(self, tr, company_name, company_id):
        ...
        tup = ('punish_content', 'execution', 'punish_department', 'publish_time', 'punish_basis', 'punish_number',
               'punish_reason', 'lawbreaking', 'company_name', 'company_id')
        values, keys = self.structure_sql_statement(tup, kwargs)
        sql = f'insert into das_tm_environmental_penalty_info {keys} value {values};'
        print(sql)
    ...
    async def parse(self, company_id, company_name, ps=20, pn=1, resp=None):
        ...
        try:
            url = f'https://www.tianyancha.com/pagination/environmentalPenalties.xhtml?ps={ps}&pn={pn}&id={company_id}&_={self.get_now_timestamp()}'
            ...
            print(f'类 - - {EnvironmentalPenalty.__name__} - - 异步请求出错：', e)
